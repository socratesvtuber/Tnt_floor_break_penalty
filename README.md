...
- 天井が無い(上部が開放された)箱を想定しているため、高さ(Y方向)だけは自動検出できず、`height`引数で明示的に指定する必要がある。**`height`は「床の上に積める空間の高さ」**を表し、実際に監視される範囲は床1層+`height`層分の合計になる(例: `height:9`なら、床を含めて合計10層が範囲になる)
